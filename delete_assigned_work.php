...
$check_stmt->execute([$work_id, $teacher_id, $dev_role]);
$check_stmt->execute();
...
/* TODO-bind_result: $check_stmt->bind_result($valid_id, $work_title); */
$_row_ = $check_stmt->fetch();
// assign: $valid_id, $work_title
$check_stmt->fetch();


// 2. ทำการ Soft Delete โดยอัปเดต is_deleted = 1
$delete_stmt = $pdo->prepare("UPDATE assignment_library SET is_deleted = 1 WHERE id = ?");
$delete_stmt->execute([$work_id]);

if ($delete_stmt->execute()) {
    
    // 3. ถ้าลบงานในคลัง ก็ต้อง Soft Delete งานที่แอสไซน์ไปแล้วด้วย
    $del_assigned = $pdo->prepare("UPDATE assigned_work SET is_deleted = 1 WHERE library_id = ?");
    $del_assigned->execute([$work_id]);
