...
switch ($action) {
    case 'create_quest':
        ...
        if (!$stmt) {
            echo json_encode(['status' => 'error', 'message' => 'SQL Error: ' . $pdo->error]);
            exit;
        }

        // Bind Parameters ("i" = integer, "s" = string, "d" = double/float)
        $stmt->execute([$teacher_id, $title, $description, $target_chem1, $target_chem2,
    ...
    case 'toggle_active':
        ...
        $check = $pdo->prepare("SELECT id FROM quests WHERE id = ? AND teacher_id = ?");
        $check->execute([$quest_id, $teacher_id]);
        $check->execute();
        if ($check->rowCount() === 0 && $userRole !== 'developer') {
            echo json_encode(['status' => 'error', 'message' => 'ไม่อนุญาตให้แก้ไขภารกิจนี้']);
            exit;
        }
        

        $stmt = $pdo->prepare("UPDATE quests SET is_active = ? WHERE id = ?");
        $stmt->execute([$new_status, $quest_id]);
