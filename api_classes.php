...
if ($method === 'POST') {
    ...
    if (!in_array($action, $safe_read_actions)) {
        ...
        if ($userRole !== 'developer' && $userRole !== 'developer') {
            http_response_code(403);
...
switch ($action) {
    ...
    case 'get_rooms':
        ...
        $rooms = [];
        $stmt = $pdo->prepare("SELECT `id`, `room`, `class_name` FROM `classes` WHERE `level` = ? AND `is_active` = 1 ORDER BY `room` ASC");
        $stmt->execute([$level]);
        $stmt->execute();
        $result = $stmt;
    ...
    case 'create_bulk_classes':
        ...
        try {
            $stmt_check = $pdo->prepare("SELECT `id` FROM `classes` WHERE `level` = ? AND `room` = ?");
            $stmt_insert = $pdo->prepare("INSERT INTO `classes` (`class_name`, `level`, `room`, `is_active`) VALUES (?, ?, ?, 1)");
            ...
            for ($r = $start_room; $r <= $end_room; $r++) {
                ...
                $stmt_insert->execute([$class_name, $level, $r]);
                $stmt_insert->execute();
                $created_count++;
            }
    ...
    case 'update_class':
        ...
        $stmt = $pdo->prepare("UPDATE `classes` SET `class_name` = ?, `teacher_id` = ? WHERE `id` = ?");
        $stmt->execute([$class_name, $teacher_id, $class_id]);
    ...
    case 'toggle_active':
        ...
        $stmt->execute([$is_active, $class_id]);
    ...
    case 'delete_class':
        ...
        $stmt_check = $pdo->prepare("SELECT id FROM users WHERE class_id = ? AND is_deleted = 0 LIMIT 1");
